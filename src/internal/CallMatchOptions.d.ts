...
export interface CallMatchOptions<T = unknown> {
	 ...
	 */
	throws(errorArgs: unknown): CallBehaviorOptions<T>;
	 ...
	 */
	returns(...values: T extends Callback ? ReturnType<T> extends [] ? ReturnType<T> : [ReturnType<T>] : ReadonlyArray<unknown>): CallBehaviorOptions<T>;
	//#endregion
